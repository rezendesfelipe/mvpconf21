...
variable "hadoop_name" {
  type        = string
  description = "Nome do cluster Hadoop no HDInsight"
}
...
variable "component_version_hadoop" {
  type        = string
  description = "Versão do Hadoop que será utilizada no cluster do HDInsight Hadoop"
}
...
variable "tier" {
  type        = string
  description = "Especifica a camada que deve ser usada para o cluster HDInsight Hadoop. Os valores possíveis são Standard ou Premium"
}
